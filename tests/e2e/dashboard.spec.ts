...
test.describe('Dashboard', () => {
  test('should display today overview and quick chat entry', async ({ page }) => {
    ...
    // Overview cards
    await expect(page.getByText('必须解决')).toBeVisible()
    await expect(page.getByText('重点关注')).toBeVisible()
    await expect(page.getByText('待追问')).toBeVisible()
    await expect(page.getByText('待了解名词')).toBeVisible()
  ...
  test('should navigate to all main pages', async ({ page }) => {
    ...
    // Knowledge
    await page.getByText('知识库').first().click()
